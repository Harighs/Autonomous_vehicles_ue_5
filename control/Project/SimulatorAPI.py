...
def game_loop(args):
    ...
    try:
        ...
        # Those parameters was tuned using iterative cycle to get the smallest overall error
        throttle_Kp = 0.155
        throttle_Ki = 0.036
        throttle_Kd = 0.0
        ...
        throttle_controller = PIDController(throttle_Kp, throttle_Ki, throttle_Kd, throttle_output_min,
                                            throttle_output_max)
...
def get_control(steer_controller : PIDController, throttle_controller : PIDController, data):
    ...
    # TODO Calculate steer error from the position and the desired trajectory
    ...
    # desired_heading = 0
    desired_heading = math.atan2(yt_points[0] - y_position, xt_points[0] - x_position)
    # steer_error = 0
    steer_error = desired_heading - math.atan2(math.sin(yaw), math.cos(yaw))
    ...
    control_logs.append([desired_heading, yaw, steer_command, desired_v, velocity, throttle])

    msgJson = data.copy()
    msgJson["brake"] = brake_command
    msgJson["throttle"] = throttle_command
    msgJson["steer"] = steer_command
    return msgJson
